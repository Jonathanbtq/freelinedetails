...
/**
 * \file    freelinedetails/admin/setup.php
 * \ingroup freelinedetails
 * \brief   Freelinedetails setup page.
 */
...
require_once '../lib/freelinedetails.lib.php';
//require_once "../class/myclass.class.php";

// Translations
$langs->loadLangs(array("admin", "freelinedetails@freelinedetails"));

// Initialize a technical object to manage hooks of page. Note that conf->hooks_modules contains an array of hook context
$hookmanager->initHooks(array('freelinedetailssetup', 'globalsetup'));
...
$item = $formSetup->newItem('FREELINEDETAILS_MYPARAM2');
...
$formSetup->newItem('FREELINEDETAILS_MYPARAM9')->setAsSelect($TField);

// Setup conf for a multiselect combo list
$item = $formSetup->newItem('FREELINEDETAILS_MYPARAM10');
$item->setAsMultiSelect($TField);
$item->helpText = $langs->transnoentities('FREELINEDETAILS_MYPARAM10');

// Setup conf for a category selection
$formSetup->newItem('FREELINEDETAILS_CATEGORY_ID_XXX')->setAsCategory('product');

// Setup conf FREELINEDETAILS_MYPARAM10
$item = $formSetup->newItem('FREELINEDETAILS_MYPARAM10');
$item->setAsColor();
...
$item = $formSetup->newItem('FREELINEDETAILS_MYPARAM11')->setAsHtml();
$item->nameText = $item->getNameText().' more html text ';
$item->fieldInputOverride = '';
$item->helpText = $langs->transnoentities('HelpMessage');
$item->cssClass = 'minwidth500';

$item = $formSetup->newItem('FREELINEDETAILS_MYPARAM12');
...
if ($action == 'updateMask') {
	...
	foreach ($dirmodels as $reldir) {
		$file = dol_buildpath($reldir."core/modules/freelinedetails/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file)) {
	...
	if ($className !== '') {
		...
		if ($module->write_file($tmpobject, $langs) > 0) {
			header("Location: ".DOL_URL_ROOT."/document.php?modulepart=freelinedetails-".strtolower($tmpobjectkey)."&file=SPECIMEN.pdf");
			return;
		} else {
	...
	if (!empty($tmpobjectkey)) {
		$constforval = 'FREELINEDETAILS_'.strtoupper($tmpobjectkey)."_ADDON";
	...
	if ($ret > 0) {
		if (!empty($tmpobjectkey)) {
			$constforval = 'FREELINEDETAILS_'.strtoupper($tmpobjectkey).'_ADDON_PDF';
...
$help_url = '';
$title = "FreelinedetailsSetup";

llxHeader('', $langs->trans($title), $help_url, '', 0, 0, '', '', '', 'mod-freelinedetails page-admin');
...
$head = freelinedetailsAdminPrepareHead();
print dol_get_fiche_head($head, 'settings', $langs->trans($title), -1, "freelinedetails@freelinedetails");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("FreelinedetailsSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$constforvar = 'FREELINEDETAILS_'.strtoupper($myTmpObjectKey).'_ADDON';
							...
							if (preg_match('/\.modules\.php$/i', $file) && preg_match('/^(pdf_|doc_)/', $file)) {
								if (file_exists($dir.'/'.$file)) {
									...
									if ($modulequalified) {
										...
										// Default
										print '<td class="center">';
										$constforvar = 'FREELINEDETAILS_'.strtoupper($myTmpObjectKey).'_ADDON_PDF';
